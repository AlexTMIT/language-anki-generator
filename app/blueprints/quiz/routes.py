from flask import Blueprint, json, render_template, request, redirect, url_for, session, current_app, flash
    ...
    try:
        if qtype == "vocab":
            ...
            payload = {
                ...
                "level": level,
                "items": bundle["items"],
                "meta": {"passage": bundle["meta"]["passage"], "quiz_lang": qlang, "n": n},
            }
            return render_template(
                "quiz/listening_run.html",
                title="Quiz: Listening Comprehension",
                audio=bundle["meta"]["audio"],  # /static/tmp/listening.mp3
                items=bundle["items"],
...
@bp.post("/quiz/grade")
def grade():
    # 1) Prefer a posted payload (used by listening), else fall back to session
    posted = request.form.get("payload")
    bundle = None
    if posted:
        try:
            bundle = json.loads(posted)
        except Exception:
            bundle = None
    ...
    lang   = bundle.get("lang") or ""   # used by all graders
    ...
    try:
        if kind == "vocab":
            ...
            verdicts = quiz_ai.eval_reading_batch(
                lang=lang, passage=passage, items=items, user_answers=user_answers
            )
            ...
            # Listening uses the reading grader with the same passage (hidden to the user)
            passage = (bundle.get("meta") or {}).get("passage", "")
            verdicts = quiz_ai.eval_reading_batch(
                lang=lang, passage=passage, items=items, user_answers=user_answers
            )
